...
def extract(archive_path: str, dest_dir: str, working_dir: str = ".") -> bool:
    ...
    print(f"[INFO] Extracting {archive_path} to {dest_dir}")
    ...
    try:
        if full_archive.endswith('.zip'):
            ...
            print(f"[WARNING] Unknown archive format for {archive_path}. Moving file directly without extracting.")
            filename = os.path.basename(full_archive)
            shutil.move(full_archive, os.path.join(full_dest, filename))
